...
def setup_motor(motor, name):
    ...
    print(f"[{name}] Setting up SPEED (Profile Velocity) mode...")
    # mode, limit_cur (A), acc_rad (rad/s^2)
    if motor.setup_mode(pyrobstride.RunMode.SPEED, 10.0, 50.0) != pyrobstride.Result.SUCCESS:
        print(f"[{name}] Failed to enter SPEED mode.")
        return False
...
def main():
    ...
    print("Starting High-Frequency Velocity Control. Press Ctrl+C to stop.")
