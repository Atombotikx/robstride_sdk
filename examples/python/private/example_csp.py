...
def setup_motor(motor, name):
    ...
    print(f"[{name}] Setting up POSITION_CSP mode...")
    # mode, limit_spd (rad/s)
    if motor.setup_mode(pyrobstride.RunMode.POSITION_CSP, 10.0) != pyrobstride.Result.SUCCESS:
        print(f"[{name}] Failed to enter POSITION_CSP mode.")
        return False
...
def main():
    ...
    print("Starting CSP dual sweep. Press Ctrl+C to stop.")
    try:
        ...
        while True:
            ...
            if status5.angle != last_angle5 or status6.angle != last_angle6:
                ...
                out = (f"\rM5 Tgt:{target_rad:>5.2f} Act:{s5.angle:>5.2f} Vel:{s5.velocity:>5.2f} Trq:{s5.torque:>4.1f} Tmp:{s5.temperature:>2.0f} F:{s5.fault_value:X} | "
                       f"M6 Tgt:{target_rad:>5.2f} Act:{s6.angle:>5.2f} Vel:{s6.velocity:>5.2f} Trq:{s6.torque:>4.1f} Tmp:{s6.temperature:>2.0f} F:{s6.fault_value:X} ")
