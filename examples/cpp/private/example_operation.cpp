...
int main() {
    ...
    std::cout << "Starting high-frequency dual sweep. Press Ctrl+C to stop." << std::endl;
    ...
    while (transport->is_connected() && keep_running) {
        ...
        auto res5 = motor5.send_mit_control(0.0f, target_rad, 0.0f, Kp, Kd);
        ...
        auto res6 = motor6.send_mit_control(0.0f, target_rad, 0.0f, Kp, Kd);
        ...
        if (s5.angle != last_angle5 || s6.angle != last_angle6) {
            std::cout << "\r" << std::fixed << std::setprecision(2)
                      ...
                      << "M6 Act:" << std::fixed << std::setprecision(2) << std::setw(5) << s6.angle 
