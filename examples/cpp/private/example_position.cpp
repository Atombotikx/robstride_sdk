...
bool setup_motor(Motor& motor, const std::string& name) {
    ...
    std::cout << "[" << name << "] Setting up POSITION (Profile Position) mode..." << std::endl;
    // mode, max_vel (rad/s), acc_set (rad/s^2), dec_set (rad/s^2)
    if (motor.setup_mode(RunMode::POSITION, 10.0f, 5.0f, 5.0f) != Result::SUCCESS) {
        std::cerr << "[" << name << "] Failed to enter POSITION mode." << std::endl;
        return false;
    }
...
int main() {
    ...
    std::cout << "Starting High-Frequency Position Control. Press Ctrl+C to stop." << std::endl;
    ...
    while (transport->is_connected() && keep_running) {
        ...
        // Period = 4 seconds
        float target = static_cast<float>(M_PI * (1.0 - std::cos(2.0 * M_PI * 0.25 * t)));
        
        // Send position target
        auto res5 = motor5.send_position_control(target);
        ...
        auto res6 = motor6.send_position_control(target);
